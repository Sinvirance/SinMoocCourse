...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      del(id) {
        let _this = this;
        Confirm.show("删除角色数据后不可恢复，确认删除?", function () {
          Loading.show();
      ...
      saveResource() {
        ...
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/system/admin/role/save-resource', {
          /* 前后端传输对象 RoleDto, 使用 @RequestBody 注解绑定 */
          id: _this.role.id,
      ...
      listRoleResource() {
        let _this = this;
        _this.$ajax.get(process.env.VUE_APP_SERVER + '/system/admin/roleResource/list-resource/' + _this.role.id).then((response)=>{
          let resp = response.data;
      ...
      listRoleUser() {
        let _this = this;
        _this.roleUsers = [];
        _this.$ajax.get(process.env.VUE_APP_SERVER + '/system/admin/roleUser/list-user/' + _this.role.id).then((res)=>{
          let response = res.data;
